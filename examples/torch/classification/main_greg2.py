...
def googlenet_criterion_fn(model_outputs: Any, target: Any, criterion: _Loss) -> torch.Tensor:
    # From https://discuss.pytorch.org/t/how-to-optimize-inception-model-with-auxiliary-classifiers/7958
    output, aux_outputs1, aux_outputs2 = model_outputs
    loss1 = criterion(output, target)
    loss2 = criterion(aux_outputs1, target)
    loss3 = criterion(aux_outputs2, target)
...
def main_worker(current_gpu, config: SampleConfig):
    ...
    compression_ctrl, model_s = create_compressed_model(model_s, nncf_config,
...
def get_all_pruned_weights(filter_indexes):
    """
    Returns list of pairs, each one of which contains the layer, 
    channel and index of the corresponding filter
    """
    weights = []
...
def get_filter_weigths(model, pruned_filter_indexes):
    """
...
def train_step2(config, model, criterion, criterion_fn, optimizer, train_loader, 
    ...
    while t <= t_pick:
        ...
        for i, (input_, target) in enumerate(train_loader):
            ...
            l2_p = reduce((lambda x, y: x + pruned_filter_weights[y][0][pruned_filter_weights[y][1]].norm(p=2)), pruned_filter_weights, 
        ...
        checkpoint_path = osp.join(config.checkpoint_save_dir, get_name(config) + '_after_greg1.pth')
        checkpoint = {
            'epoch': config.global_epochs,
            'arch': "unknown",
            'state_dict': model.state_dict(),
